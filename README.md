...
4. 确认客户是外企还是国内客户：外企默认做中英文双语，国内客户默认纯中文。
5. 把粗时间线细化成按天排布的 small times：研究设计、招募、田野、分析、Topline、报告、修改、终版交付等。
6. 只问少量真正影响排期的问题。
7. 先在聊天里给一版文字版 timeline 草稿。
8. 用户确认或修改后，再生成 Excel。

## 重要规则

- Proposal 里的粗时间线是主轴，不随便重排。
- 除非用户明确要求按周 / 按月看，timeline 默认优先一天一天排。
- 外企客户默认中英文双语；国内客户默认纯中文。
- 交付、确认、反馈、沟通节点默认避开周末和法定节假日。
- 甘特图里黄色表示 Working process，绿色表示 Delivery / 确认 / 递交当天。
- 线上 / 线下一对一深访默认是过程任务，不作为 delivery 节点；除非它本身就是正式交付或关键确认节点。
